...
export default function WildcardBiddingPage() {
  ...
  if (!applied) return <div className="stack"><PageHeading eyebrow="Wildcard · Slot bidding" title="Spectator mode" /><Card className="center-card"><WaitingState text="Only teams that applied can place a wildcard slot bid." /></Card><Card><h2>Live ranking</h2><Leaderboard entries={entries} currentTeamId={dashboard.team.id} cutoff={slots} cutoffLabel="Wildcard cut-off" /></Card></div>
  ...
  return <div className="stack wildcard-bidding-page">
    ...
    <div className="two-column bid-layout">
      ...
      <Card className="bid-panel"><h2>Place your slot bid</h2><form className="form" onSubmit={submit}><label className={!isLeader ? 'is-locked' : ''}><span>Bid amount</span><input type="number" inputMode="numeric" min={1} max={dashboard.wallet.balance} value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="250" disabled={!isLeader} required /></label><Button type="submit" disabled={!isLeader || working || !amount}>{working ? 'Placing bid…' : dashboard.wildcardBidAmount == null ? 'Place slot bid' : 'Update slot bid'}</Button></form><p className="muted bid-rules">Higher bids rank first. For equal bids, the earlier final bid timestamp ranks first.</p><p className="notice">Winning bids are deducted only when the organizer closes bidding.</p>{!isLeader && <p className="notice">Only your team leader can place or update the bid.</p>}{message && <p className={message.type === 'success' ? 'success' : 'error'} role="status">{message.text}</p>}</Card>
    </div>
