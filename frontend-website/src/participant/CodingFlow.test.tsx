...
describe('merged coding and submission flow', () => {
  ...
  it('enables repository updates for the leader throughout Coding', async () => {
    ...
    expect(host.textContent).toContain('Team coins')
    expect(host.textContent).toContain('750')
    expect(host.textContent).toContain('Submission status')
    expect(host.textContent).not.toContain('Time remaining')
    expect(host.textContent).not.toContain('01:00:00')
    expect(host.querySelector('a[href="https://github.com/team-seven/final"]')).not.toBeNull()
    expect(host.querySelector('.repository-submission form')).not.toBeNull()
