...
const { clearAccessToken, getAccessToken, setAccessToken, shouldClearAccessTokenForStatus } = await import('./participantToken.ts')
...
const loginSource = readFileSync(new URL('./authService.ts', import.meta.url), 'utf8')
assert.doesNotMatch(loginSource, /participant\/dashboard/, 'login must not depend on an immediate dashboard request')
assert.equal(shouldClearAccessTokenForStatus(401), true)
assert.equal(shouldClearAccessTokenForStatus(503), false, 'transient 503 responses must preserve a valid participant token')
assert.equal(shouldClearAccessTokenForStatus(0), false, 'network failures must preserve a valid participant token')
