...
import {
  ...
  confirmWildcardSlots, startWildcardSlotBidding, closeWildcardSlotBidding, finalizeWildcard, endWildcardSelectionTurn, endWildcard, endWildcardFinalChoice,
...
const labels = {
  ...
  WILDCARD_SELECTION: "Wildcard selection", WILDCARD_FINAL_CHOICE: "Wildcard final choice", CODING: "Coding", SUBMISSION: "Submission",
...
function WildcardControlPage({ state, config, remaining, onConfig, socketConnected, realtimeEvent }) {
  ...
  useEffect(() => {
    ...
    if (data?.status === "PROBLEM_SELECTION" || data?.status === "FINAL_CHOICE" || data?.status === "COMPLETE") setTab("selection");
  }, [data?.status]);
  const run = async (operation, success) => {
    setWorking(true); setError(""); setNotice("");
  ...
  const stageCopy = {
    ...
    FINAL_CHOICE: "Winners are choosing their final problem",
  ...
  return <section className="round-console wildcard-console">
    ...
    {tab === "selection" && <div className="wildcard-panel-grid">
      ...
      {(data.status === "FINAL_CHOICE" || data.status === "COMPLETE") && data.final_choice && <section className="wildcard-stage-card wildcard-final-choice"><div className="round-section-heading"><div><span className="eyebrow">FINAL PROBLEM CHOICE</span><h3>{data.final_choice.confirmed_count} / {data.final_choice.confirmed_count + data.final_choice.pending_count} confirmed</h3><p>Winning bids stay deducted. Choosing Round 1 does not refund; choosing Wildcard does not charge again.</p></div><div className="round-inline-actions">{data.final_choice.open && <button className="danger-button" disabled={working} onClick={() => window.confirm("End the choice period? Pending teams default to their Round 1 problem. Paid Wildcard bids are not refunded.") && run(endWildcardFinalChoice, "Choice period ended. Pending teams defaulted to Round 1.")}>End choice period</button>}</div></div><div className="wildcard-qualification-list">{data.final_choice.entries.length ? data.final_choice.entries.map((row) => <article key={row.team_id}><strong>#{row.rank}</strong><div><b>{row.team_name}</b><span>Round 1: {row.round1_problem ? `#${row.round1_problem.problem_number} — ${row.round1_problem.title}` : "—"} · Wildcard: {row.wildcard_problem ? `#${row.wildcard_problem.problem_number} — ${row.wildcard_problem.title}` : "—"}</span><span>Winning bid: {row.winning_bid ?? "—"} · Coins paid: {row.coins_paid ?? "—"} · Final: {row.final_choice ?? "—"}{row.confirmed_at ? ` · ${row.confirmed_at}` : ""}</span></div><em className="selection-status">{row.status}</em></article>) : <div className="round-empty"><strong>No Wildcard winners</strong><p>Finalize slot bidding to determine winners.</p></div>}</div></section>}
