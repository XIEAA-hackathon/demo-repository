...
export function WildcardControlPage({ state, config, remaining, onConfig, socketConnected, realtimeEvent }) {
  ...
  useEffect(() => {
    ...
    if (data?.status === "PROBLEM_SELECTION") setTab("selection");
    if (["FINAL_CHOICE", "COMPLETE"].includes(data?.status)) setTab("final-choice");
  }, [data?.status]);
  const run = async (operation, success) => {
    setWorking(true); setError(""); setNotice("");
  ...
  return <section className="round-console wildcard-console">
    ...
    <div className="wildcard-tabs" role="tablist" aria-label="Wildcard stages">
      {[["applications", "1", "Application"], ["bidding", "2", "Slot bidding"], ["selection", "3", "Problem selection"], ["final-choice", "4", "Final choice"]].map(([id, number, label]) => <button key={id} role="tab" aria-selected={tab === id} className={tab === id ? "active" : ""} onClick={() => setTab(id)}><span>{number}</span>{label}</button>)}
    </div>
