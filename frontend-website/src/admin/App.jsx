...
function RecoveryPage({ onGlobalSync, onNavigate }) {
  ...
  const resetCredentials = async () => { setWorking(true); setError(""); setNotice(""); try { const result = await resetRegistrationCredentials(credentialResetConfirmation); setCredentialResetSummary(result); setCredentialResetConfirmation(""); setData(await getRecoveryState()); setNotice("Event data and imported participant credentials reset successfully."); await onGlobalSync(); } catch (cause) { setError(cause.message || "Participant credential reset failed."); await load(); } finally { setWorking(false); } };
...
function RegistrationImport() {
  ...
  return <section className="registration-import">
    ...
    <section className="participant-credential-reset">
      ...
      <button className="danger-button" disabled={working || resetConfirmation !== "RESET CREDENTIALS"} onClick={() => window.confirm("Reset the current event and remove all imported participant credentials? This cannot be undone.") && void resetCredentials()}>Reset participant credentials</button>
