...
const labels = {
  ...
  WILDCARD_SELECTION: "Wildcard selection", WILDCARD_FINAL_CHOICE: "Final problem choice", CODING: "Coding",
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <nav className="sidebar-nav">
          ...
          {[["dashboard", "Overview", "⌂"], ["round1", "Round 1", "1"], ["change-problem", "Change Problem", "C"], ["wildcard", "Wildcard", "W"], ["team-allotment", "Team Allotment", "T"], ["coding", "Coding Round", "C"], ["judging", "Judging", "J"], ["recovery", "Recovery", "R"]].map(([id, label, icon]) => (
...
function WildcardControlPage({ state, config, remaining, onConfig, socketConnected, realtimeEvent }) {
  ...
  useEffect(() => {
    ...
    if (["PROBLEM_SELECTION", "FINAL_CHOICE", "COMPLETE"].includes(data?.status)) setTab("selection");
  }, [data?.status]);
  const run = async (operation, success) => {
    setWorking(true); setError(""); setNotice("");
  ...
  const stageCopy = {
    ...
    FINAL_CHOICE: "Wildcard winners are choosing their final problem",
...
export function CodingRoundAdminPage({ socketStatus = "disconnected", realtimeEvent = null, state = null, config = null, remaining = 0, onConfig = () => {}, onGlobalSync = async () => {} }) {
  ...
  return <section className="submission-admin coding-round-admin">
    <header className="submission-admin__header"><div><span className="eyebrow">EVENT / CODING ROUND</span><h2>Coding Round</h2><p>Control the coding timer and track each team’s final GitHub repository.</p></div><div className="submission-admin__actions"><button className="secondary-button" disabled={working || !data.export_available} title={data.export_available ? "Download final event results" : "Available after submissions are closed"} onClick={() => void downloadFinalExport()}>{working ? "WORKING…" : "EXPORT EXCEL / CSV"}</button><button className={data.open ? "danger-button" : "primary-button"} disabled={working || (!data.open && state?.event_state !== "CODING")} onClick={() => run(data.open ? closeSubmissions : openSubmissions, data.open ? "Submissions closed. Judging wait started." : "Submissions opened without changing the coding timer.")}>{data.open ? "Close submissions" : "Open submissions"}</button></div></header>
    {error && <div className="global-error" role="alert">{error}</div>}{notice && <div className="admin-notice">{notice}</div>}
    <section className="wildcard-stage-card coding-timer-panel"><div><span className="eyebrow">CODING TIMER SETTINGS</span><h3>{formatTime(remaining)} remaining</h3><p>Configured duration: {formatTime(config?.coding_duration_seconds ?? 0)}. Submission controls do not restart or stop this timer.</p></div>{config && <label>Coding duration (seconds)<input type="number" min="1" value={config.coding_duration_seconds} onChange={(event) => onConfig({ ...config, coding_duration_seconds: Number(event.target.value) })} /></label>}<div className="round-inline-actions"><button className="secondary-button" disabled={working || !config} onClick={() => void saveCodingDuration()}>Save duration</button><TimerButtons state={state} remaining={remaining} run={runTimer} /></div></section>
    {!data.export_available && <div className="admin-notice">Final export is available after submissions are closed.</div>}
...
function RecoveryPage({ onGlobalSync, onNavigate }) {
  ...
  const resetCredentials = async () => { setWorking(true); setError(""); setNotice(""); try { const result = await resetRegistrationCredentials(credentialResetConfirmation); setCredentialResetSummary(result); setCredentialResetConfirmation(""); setData(await getRecoveryState()); setNotice("Imported participant registration cleared. The event lifecycle was not reset."); await onGlobalSync(); } catch (cause) { setError(cause.message || "Participant credential reset failed."); await load(); } finally { setWorking(false); } };
...
function RegistrationImport() {
  ...
  return <section className="registration-import">
    ...
    <section className="participant-credential-reset">
      <div><strong>Participant credential reset</strong><h3>Clear imported participant registration</h3><p>Remove import-created participant accounts, imported teams, registration rows, and active participant sessions.</p><p>The event lifecycle is not reset. Permanent Admin, demo/system, and leaderboard display accounts remain unchanged.</p></div>
      ...
      <button className="danger-button" disabled={working || resetConfirmation !== "RESET CREDENTIALS"} onClick={() => window.confirm("Remove imported participant accounts and teams? The event lifecycle will not be reset.") && void resetCredentials()}>Reset participant credentials</button>
      {resetError && <p className="participant-credential-reset__error" role="alert">{resetError}</p>}
      {resetResult && <div className="participant-credential-reset__result" role="status"><strong>Credential reset complete</strong><span>Participant accounts removed: {resetResult.deleted.participant_accounts} · Imported teams removed: {resetResult.deleted.teams} · Event lifecycle reset: No</span></div>}
    </section>
  </section>;
}
