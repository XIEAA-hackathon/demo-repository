      ...
      <div className="problem-grid">
        {problems.map((item) => (
          <label key={item.id} className={`card selectable ${selected === item.id ? 'is-selected' : ''}`}>
            <input type="radio" name="problem" checked={selected === item.id} onChange={() => setSelected(item.id)} />
            <span><small>Problem #{String(item.number).padStart(2, '0')}</small><strong>{item.title}</strong><small>{item.summary}</small></span>
          </label>
        ))}
      </div>
      <Card className="action-row"><Button onClick={() => setPendingChange(true)} disabled={!selected}>Choose replacement problem</Button></Card>
