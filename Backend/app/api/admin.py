...
async def update_event_config_admin(
    ...
    if "wildcard_final_choice_seconds" in data and not 5 <= data["wildcard_final_choice_seconds"] <= 300:
        raise HTTPException(status_code=400, detail="wildcard_final_choice_seconds must be between 5 and 300")
